...
namespace name_sorter.Services
{
    ...
    public class NameSortProcessor(IFileService fileService, INameSorter nameSorter)
    {
        ...
        public async Task SortNamesAsync(string inputFilePath, string outputFilePath, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                Console.WriteLine($"Successfully processed {names.Count} names. Sorted names written to: {outputFilePath}");
            }
            catch (Exception ex)
            {
